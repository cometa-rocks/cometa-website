...
header("C.301: enter CaptchHandler");
...
try {
  ...
  header("C.302a: ".$_GET['get']);
  header("C.302c: $commandString");
  $command = BDC_CaptchaHttpCommand::FromQuerystring($commandString);
  header("C.303: xx ".serialize($command));
  switch ($command) {
    case BDC_CaptchaHttpCommand::GetImage:
      header("C.304a: GetImage");
      GetImage();
      header("C.304b: GetImage");
      break;
    ...
    case BDC_CaptchaHttpCommand::GetP:
      ...
      break;
    default:
      header("C.305: BadRequest");
      BDC_HttpHelper::BadRequest('command');
      break;
  }

} catch (Exception $e) {
  header("C.306: Exception");
